...
export const GiftingSection: React.FC = () => {
  ...
  const handleCopyMpesa = () => {
    navigator.clipboard.writeText(WEDDING_DETAILS.tillNumber || WEDDING_DETAILS.mpesaNumber);
    setCopiedMpesa(true);
      ...
      <section id="gifting" className="max-w-4xl mx-auto">
        ...
        <div className="max-w-md mx-auto bg-[#1E3A2B] text-[#FDF8F2] rounded-2xl p-5 border border-[#D4A359] shadow-md text-left space-y-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <span className="block font-serif text-base font-bold text-white">
                Buy Goods & Services • {WEDDING_DETAILS.mpesaName}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between bg-[#2C4C3B] p-3 rounded-xl border border-[#D4A359]/40">
            <div>
              <span className="block text-[10px] text-[#F5ECE0]/80 uppercase font-semibold">Till Number</span>
              <span className="font-serif text-2xl font-bold text-[#D4A359] tracking-wider">
                {WEDDING_DETAILS.tillNumber}
              </span>
            </div>

            <button
              onClick={handleCopyMpesa}
              className="px-3.5 py-2 rounded-lg bg-[#C15C3D] hover:bg-[#A8482A] text-white text-xs font-semibold flex items-center gap-1.5 transition-colors shadow-2xs"
            >
              {copiedMpesa ? <Check className="w-3.5 h-3.5 text-white" /> : <Copy className="w-3.5 h-3.5" />}
              <span>{copiedMpesa ? 'Copied!' : 'Copy Till'}</span>
            </button>
          </div>
